...
while IFS="	" read -r account admin network; do
    ...
    $ctl user run "$account" network create -addr irc+insecure://ergo:6667 -name "$network" -nick "$account" >/dev/null 2>&1 \
        || $ctl user run "$account" network update "$network" -addr irc+insecure://ergo:6667 -nick "$account" >/dev/null
 done < "$users"
wait "$pid"
